...
public class ObjectTree {
    ...
    public static String dumpXML(Object root, int levels, int sizeLimit) {
        StringBuilder sb = new StringBuilder();
        Node nodeTree = Node.create(root);
        printXMLTree("", sb, nodeTree, levels, sizeLimit);
        return sb.toString();
    }

    public static String dump(Object root, int levels, int sizeLimit) {
        StringWriter sw = new StringWriter();
        PrintWriter pw = new PrintWriter(sw);
        Node nodeTree = Node.create(root);
        printTree(new StringBuilder(), new StringBuilder(), pw, nodeTree, levels, sizeLimit);
        pw.flush();
        return sw.toString();
    }
    ...
    public static void dump(PrintWriter pw, Object root) {
        Node nodeTree = Node.create(root);
        printTree(new StringBuilder(), new StringBuilder(), pw, nodeTree, -1, -1);
    }
    ...
    public static class Node {
        ...
        public void setDeepSize(long deepSize) {
    ...
    private static void printTree(StringBuilder prefix, StringBuilder line,
                                  PrintWriter pw, Node node, int levels, int sizeLimit) {
        ...
        if (node.hasChildren() && levels >= 0) {
            ...
            Collections.sort(node.getChildren(), Comparator.comparing(Node::getDeepSize).reversed());

            for (Iterator<Node> i = node.getChildren().iterator(); i.hasNext(); ) {
                Node next = i.next();
                if (sizeLimit > 0 && next.deepSize < sizeLimit) {
                    continue;
                }
                line.append(prefix.toString());
                line.append("+- ");
                prefix.append(i.hasNext() ? "|  " : "   ");
                printTree(prefix, line, pw, next, levels, sizeLimit);
                prefix.setLength(pLen);
            }
        }
    }

    private static void printXMLTree(String prefix, StringBuilder sb, Node node, int levels, int sizeLimit) {
        sb.append(prefix);
        sb.append("<node ");
        sb.append("deepSize=\"" + node.getDeepSize() + "\" ");
        sb.append("name=\"" + StringEscapeUtils.escapeXml11(node.getName()) + "\" ");
        sb.append(">");
        ...
        if (node.hasChildren() && levels >= 0) {
            ...
            for (Iterator<Node> i = node.getChildren().iterator(); i.hasNext(); ) {
                ...
                printXMLTree(prefix, sb, next, levels, sizeLimit);
